  ...
  render() {
    ...
    if (subnav) {
      const isNavItemSelected =
        active || (!!selected && selected === this.props.eventKey);

      return (
        <Component
          role="presentation"
          className={cx(className, "sidenav-subnavitem", {
        ...
        >
          <div
            {...props}
            className={cx(navitemClassName, "navitem")}
            style={navitemStyle}
            disabled={disabled}
            role="menuitem"
            tabIndex="-1"
          ...
          >
            {navIcon && (
              <div
                {...navIconProps}
                className={cx(navIconClassName, "navicon")}
              />
            )}
            {navText && (
              <div
                {...navTextProps}
                className={cx(navTextClassName, "navtext")}
              />
            )}
          </div>
        </Component>
    ...
    const navItems = React.Children.toArray(children)
      ...
      .map(child => {
        if (
          child.props.active ||
          (!!selected && selected === child.props.eventKey)
        ) {
    ...
    const others = React.Children.toArray(children).filter(child => {
      if (
        ...
        (this.isNavIcon(child) ||
          this.isNavText(child) ||
          this.isNavItem(child))
      ) {
        return false;
      }
      return true;
    ...
    return (
      <Component
        role="presentation"
        className={cx(className, "sidenav-navitem", {
          ["selected"]: isNavItemSelected,
          ["highlighted"]: isNavItemHighlighted,
          ["expandable"]: isNavItemExpandable,
      ...
      >
        ...
        {navItems.length > 0 && (
          <div
            role="menu"
            className={cx(subnavClassName, "sidenav-subnav")}
            style={subnavStyle}
          >
            <Component
              role="heading"
              className={cx("sidenav-subnavitem", {
                ["highlighted"]: active,
                ["disabled"]: disabled,
              })}
              style={style}
